...
`appcore-gateway` : capability de gateway multi-tenant pour un accès Internet sécurisé sans exposer directement les workers.

## Responsabilités

Il multiplexe le trafic WebSocket/RPC, valide des peer tokens à durée courte, route par tenant et n'interprète jamais les payloads métier opaques.
