...
`appcore-gateway`: Multi-tenant gateway capability for secure Internet access without directly exposing workers.

## Responsibilities

It multiplexes WebSocket/RPC traffic, validates short-lived peer tokens, routes by tenant, and never interprets opaque business payloads.
