...
`appcore-provider`: Implementation-neutral provider roles, plans, factories, registry, secret resolution, coordination, and job contracts.

## Responsabilités

Unavailable providers fail bootstrap; the registry never silently substitutes another implementation.
