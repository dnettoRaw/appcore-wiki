...
`appcore-api`: Runtime HTTP command/query/status DTOs, router, and host.

## Responsibilities

Stable routes include health, status, command, and query V1 endpoints. Protected routes reject missing, malformed, or duplicate `Authorization` headers.
