...
`appcore-supervisor`: Dependency-aware lifecycle management for runtime services, restart policies, budgets, watchdog, diagnostics, and adapters.

## Responsabilidades

It supervises services inside the process; the operating-system process remains owned by systemd, launchd, SCM, containers, or another service manager.
