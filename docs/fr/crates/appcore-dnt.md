...
`appcore-dnt`: Versioned encrypted binary envelope for portable local records, backup bundles, snapshots, outbox files, and secret material.

## Responsabilités

DNT authenticates canonical headers as AEAD additional data and keeps cryptographic keys outside the file.
