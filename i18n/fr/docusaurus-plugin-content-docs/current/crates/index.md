...
La bêta officielle [`appcore-filemaker 0.1.0-beta.1`](./appcore-filemaker) est
publiée sur crates.io et développée sur la branche Runtime `beta`. Elle ne
fait pas partie du graphe stable.
Son [bridge IA](./appcore-filemaker-ai) et son
[adaptateur CLI](./appcore-filemaker-cli) facultatifs sont aussi publiés comme
prereleases beta séparées `0.1.0-beta.1`.
...
le tag Git historique reste uniquement une preuve d'audit immuable. Les
...
| `0.1.0-alpha.4` | `appcore-sync-sqlite` ; successeur propre de l'archive alpha.3 retirée dont les dépendances storage et sync référençaient la ligne alpha erronée |
